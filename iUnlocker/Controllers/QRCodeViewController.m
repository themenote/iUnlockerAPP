...
@interface QRCodeViewController () <SFSafariViewControllerDelegate>
...
- (void)setBackground {
    UIView *bv1 = [[UIView alloc] initWithFrame:(CGRect){0, 0, SCREENWIDTH, 150}];
    bv1.alpha = 0.8f;
    bv1.backgroundColor = RGB(0x333333);
    [self.view addSubview:bv1];
    
    UIView *bv2 = [[UIView alloc] initWithFrame:(CGRect){0, 150, (SCREENWIDTH-285)/2, 285}];
    bv2.alpha = 0.8f;
    bv2.backgroundColor = RGB(0x333333);
    [self.view addSubview:bv2];
    
    UIView *bv3 = [[UIView alloc] initWithFrame:(CGRect){(SCREENWIDTH+285)/2, 150, (SCREENHEIGHT-285)/2, 285}];
    bv3.alpha = 0.8f;
    bv3.backgroundColor = RGB(0x333333);
    [self.view addSubview:bv3];
    
    UIView *bv4 = [[UIView alloc] initWithFrame:(CGRect){0, 435, SCREENWIDTH, SCREENHEIGHT-435}];
    bv4.alpha = 0.8f;
    bv4.backgroundColor = RGB(0x333333);
...
- (void)setScanFrame {
    self.scanImageView = [[UIImageView alloc] initWithFrame:(CGRect){(SCREENWIDTH-285)/2, 150, 285, 285}];
    self.scanImageView.image = IMG(@"scan_frame.png");
    [self.view addSubview:self.scanImageView];
}

- (void)setScanAnimation {
    self.animationImageView = [[UIImageView alloc] initWithFrame:(CGRect){(SCREENWIDTH-285)/2, 150, 285, 2}];
    self.animationImageView.image = [UIImage imageNamed:@"scan_animation.png"];
    [self.view addSubview:self.animationImageView];
}

- (void)setDescription {
    UILabel *description = [[UILabel alloc] initWithFrame:(CGRect){0, 450.f, SCREENWIDTH, 16.f}];
    [description setTextColor:RGBA(0xffffff, .6)];
...
- (void)setCamera {
    if (self.session == nil) {
        ...
        [self.output setRectOfInterest:(CGRect){(150)/SCREENHEIGHT, ((SCREENWIDTH-285)/2)/SCREENWIDTH, 285/SCREENHEIGHT, 285/SCREENWIDTH}];
...
- (void)setCancelButton {
    UIButton *button = [[UIButton alloc] initWithFrame:(CGRect){(SCREENWIDTH-150.f)/2, (SCREENHEIGHT-50.f), 150.f, 35.f}];
    [button.titleLabel setFont:[UIFont fontWithName:@"Helvetica" size:14.0]];
    [button setTitle:@"取消扫码" forState:UIControlStateNormal];
    [button setTitleColor: RGB(0xffffff) forState:UIControlStateNormal];
    [button setTitleColor: RGB(0xaaaaaa) forState:UIControlStateSelected];
...
- (void)handleScanResult:(NSString *)result {
    ...
    if ([result hasPrefix:@"iunlocker://"]) {
        targetController = [[LoginViewController alloc] init];
        [targetController setUrl:result];
        
        [self.navigationController pushViewController:targetController animated:YES];
    } else if ([result hasPrefix:@"iunlocker-auth://"]) {
        targetController = [[OauthViewController alloc] init];
        [targetController setUrl:result];
